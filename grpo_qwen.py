"""
GRPO (Group Relative Policy Optimization): Qwen2.5-3B Reasoning
================================================================
Fine-tune Qwen2.5-3B (after SFT) using GRPO with Format and Accuracy rewards.
This script uses trl's GRPOTrainer.

Usage:
    python grpo_qwen.py --model_name_or_path ./sft_output --dataset_name AI-MO/NuminaMath-TIR
"""
...
def format_reward_func(completions, **kwargs) -> list[float]:
    """
    Reward function that checks if the completion follows the <think>...</think> format.
    """
    rewards = []
    for completion in completions:
        text = completion[0]["content"] if isinstance(completion, list) else completion
        if "<think>" in text and "</think>" in text:
            # Basic validation: `<think>` must appear before `</think>`
